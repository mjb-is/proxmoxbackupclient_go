...
This is **mjb-is's fork** of the upstream project — repo: **https://github.com/mjb-is/proxmoxbackupclient_go**. Focus areas: creating functionality and flow that would allow PBS to be the single server based backup system in my homelab, which currently uses a mix Proxmos Backup Server with Proxmox VE VM and Linux host backups, Backup for Workgroups for Windows servers file based and bare metal recovery, FastFileSync/Rsync incremental file mirros, and CloneZilla/RescueZilla. Hardening the Windows/Linux GUI client and building a fully automated bare-metal restore path on top of Clonezilla. Highlights over upstream:
...
- **New: a fully automated "PBS Bare Metal Restore" wizard** for the Clonezilla live ISO — see [below](#clonezilla-live-iso-bare-metal-restore). - Test restore of 32GiB Windows machine from 'power on' into boot ISO, through to restored Windows login in just 8 minutes. Similar for 40GiB Linux Mint restore in 10 minutes to login prompt. Both tested on isolated VMs, still to be tested on bae metal.  

Anything of general use gets sent upstream as a PR (like #85 above) rather than kept fork-only; day-to-day fork-specific work stays here. The original author may shoose to merge anything else from this repo as they see fit under GPL.
